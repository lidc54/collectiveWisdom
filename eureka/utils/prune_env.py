...
def extract_self_variable(obs_string, tip_str):
    def all_property(obs_):
        out = []
        obs = re.split(" |\t|\n|\[|\)|\(|,", obs_)
        for p in obs:
            if "self." not in p:
    ...
    all_self_property = all_property(obs_string)
    exclude_self_property = all_property(obs_string.split("=")[0])# 去除等号前的变量
    # print(all_self_property-exclude_self_property)
    msg = " ".join(all_self_property)
    # print(all_self_property-exclude_self_property)
    msg2 = " ".join(all_self_property-exclude_self_property)
    # return msg2 
    return tip_str.replace(
        "(namely, variables that have prefix self.).", f"(namely, {msg2}.)."
    )


def new_code_output_tip(task_obs_code_string, tip_str):
    msg = prune_obs(task_obs_code_string)
    print(msg)
    return tip_str.replace(
        "(namely, variables that have prefix self.).", f"(namely, {msg}.)."
    )
